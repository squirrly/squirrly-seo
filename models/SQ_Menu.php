	...
	public function addMeta($param=null){
		...
		if (is_array($this->meta)){
			...
			if ($this->meta[0] <> '' && $this->meta[1] <> ''){
				...
				if (!isset($this->meta[5])) $this->meta[5] = null;
				if (!isset($this->meta[6])) $this->meta[6] = null; //no arg
				//print_r($this->meta);
  ...
  function resizeImage($maxwidth,$maxheight,$preserveAspect=true) {
    ...
    if ($width>$maxwidth && $height>$maxheight) {
      ...
      if ($preserveAspect)
      {
          ...
          // CHANGED EXCEPTION TO RETURN FALSE
          if (imagecopyresampled($dest,$this->imgH,0,0,0,0,$newwidth,$newheight,$width,$height)==false) return false; // throw new Exception('Couldn\'t resize image!');
      }else{
          $dest=imagecreatetruecolor($maxwidth, $maxheight);
          $this->applyTransparency($dest);
          // CHANGED EXCEPTION TO RETURN FALSE
          if (imagecopyresampled($dest,$this->imgH,0,0,0,0,$maxwidth,$maxheight,$width,$height)==false) return false; // throw new Exception('Couldn\'t resize image!') ;
      }
      $this->imgH=$dest;
    }
  }
...
class Model_SQ_Icon{
    ...
    function __construct($image, $newfile){
        ...
        if($is_transparent) $color_count--;

        if($color_count==0) {$color_count=0; $bit_count=24;};
        if(($color_count>0)and($color_count<=2)) {$color_count=2; $bit_count=1;};
        if(($color_count>2)and($color_count<=16)) { $color_count=16; $bit_count=4;};
        if(($color_count>16)and($color_count<=256)) { $color_count=0; $bit_count=8;};
        ...
        $planes=0;
        if($bit_count >= 8) $planes=1;

        $ret.=$this->jpexs_inttoword($f,$planes);//PLANES
        if($bit_count>=8) ($w_bit_count = $bit_count);
        if($bit_count==4) $w_bit_count=0;
        if($bit_count==1) $w_bit_count=0;
        ...
        if($bit_count<=8){
         for($y=$height-1;$y>=0;$y--){
            ...
            for($x=0;$x<$width;$x++){
              ...
              if($color==-1) $color=0;
              if($color>pow(2,$bit_count)-1) $color=0;
