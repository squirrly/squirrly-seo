...
class Model_SQ_PostsList{
    ...
    private function packBriefTotal($totals){
        if (!is_array($totals) || count($totals) == 0) return false;
    ...
    private function packBriefProgress($history){
        if (!is_array($history) || count($history) == 0) return false;
        ...
        foreach ($history as $date){
            if (!is_array($date->values)) continue;
    ...
    private function packBriefSERP($totals){
        if (!is_array($totals)) return false;
...
/******************* TOTALS ****************************/
    ...
    public function packOthersSERP(){
        ...
        if (!is_array($ranks['others'])) return false;
